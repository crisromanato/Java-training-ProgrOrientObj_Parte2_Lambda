package ProgramacaoOrientadaObjetos_Parte2_Lambda.Cap20_ProgFuncional_ExpLambda.Consumer.Case4_ExpLambdaDeclarada.application;
 ...
 */
public class ProgrConsumer4 {
    public static void main (String[] args) {

        Locale.setDefault (Locale.US);
       List<ProductConsumer4> list = new ArrayList<> ();

        System.out.println ("Consumer usando Expressao Lambda declarada ");
        
    list.add (new ProductConsumer4 ("Tv", 900.00));
    list.add (new ProductConsumer4 ("Mouse", 50.00));
    list.add (new ProductConsumer4 ("Tablet", 350.50));
    list.add (new ProductConsumer4 ("HD Case", 80.90));
...
declarar duas variaveis, uma para receber o fator de calculo do aumento
dos produtos e um consumer para fazer via expressao lnmbda
no list.forEach mudamos pra receber a variavel cons.

Lembrando que como tem apenas uma linha a funcao do consumer as chaves
sao opcionais.

com chaves

Consumer<ProductConsumer4> cons = p4 ->{
