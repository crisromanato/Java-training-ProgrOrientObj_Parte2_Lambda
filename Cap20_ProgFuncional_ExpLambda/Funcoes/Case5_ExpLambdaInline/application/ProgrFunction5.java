package ProgramacaoOrientadaObjetos_Parte2_Lambda.Cap20_ProgFuncional_ExpLambda.Funcoes.Case5_ExpLambdaInline.application;
 ...
 */
public class ProgrFunction5 {
    public static void main (String[] args) {

        Locale.setDefault (Locale.US);
       List<ProductFunction5> list = new ArrayList<> ();

        System.out.println ("Consumer usando expressao lambda inline ");
        
    list.add (new ProductFunction5 ("Tv", 900.00));
    list.add (new ProductFunction5 ("Mouse", 50.00));
    list.add (new ProductFunction5 ("Tablet", 350.50));
    list.add (new ProductFunction5 ("HD Case", 80.90));
...
O map recebe como argumento uma expressao lambda inline , entao vamos ter que pegar
...
Function<ProductFunction4, String> func =
                ...
                a parte inline é: p -> p.getName ().toUpperCase ()

Na funcao map colocamos a parte do inline.
...
List<String> names = list.stream().map
        (p -> p.getName ().toUpperCase ()).collect (
                Collectors.toList ());
